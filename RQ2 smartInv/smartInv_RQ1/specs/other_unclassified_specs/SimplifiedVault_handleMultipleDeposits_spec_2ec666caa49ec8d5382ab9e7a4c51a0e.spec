...
rule ValidateMultipleDepositsConsistency() {
    uint256 $fnftId;
    uint256 $newFNFTId;
    uint256 $amount;
    ...
    if($newFNFTId != 0) {
        ...
        assert(fnfts[$newFNFTId].depositAmount != newFNFTDepositAmountBefore);
    } else {
        assert(fnfts[$fnftId].depositAmount == depositAmountBefore + $amount || fnfts[$fnftId].depositAmount == $amount);
    }
}}