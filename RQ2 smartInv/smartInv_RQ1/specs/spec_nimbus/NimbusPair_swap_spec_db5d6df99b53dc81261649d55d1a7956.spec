...
rule EnsureSwapSafety() {
    // Symbolic variables for input parameters
    uint amount0OutSym;
    uint amount1OutSym;
    address toSym;

    // Symbolic variables representing initial state
    uint112 initialReserve0Sym;
    uint112 initialReserve1Sym;
    uint initialBalance0Sym;
    uint initialBalance1Sym;

    // Preconditions
    require(initialReserve0Sym > 0 && initialReserve1Sym > 0);
    require(initialBalance0Sym >= initialReserve0Sym && initialBalance1Sym >= initialReserve1Sym);

    // Simulate swap 
    swap(amount0OutSym, amount1OutSym, toSym, "");

    // Retrieve final state
    uint finalBalance0 = IERC20(token0).balanceOf(address(this));
    uint finalBalance1 = IERC20(token1).balanceOf(address(this));
    (uint112 finalReserve0, uint112 finalReserve1,) = getReserves();

    // Compute actual amount in
    uint amount0In = finalBalance0 > initialBalance0Sym ? finalBalance0 - initialBalance0Sym + amount0OutSym : amount0OutSym;
    uint amount1In = finalBalance1 > initialBalance1Sym ? finalBalance1 - initialBalance1Sym + amount1OutSym : amount1OutSym;

    // Postconditions
    assert(finalBalance0 >= initialBalance0Sym);
    assert(finalBalance1 >= initialBalance1Sym);

    assert(amount0OutSym <= initialReserve0Sym);
    assert(amount1OutSym <= initialReserve1Sym);

    assert(amount0In > 0 || amount1In > 0);
    assert(amount0OutSym > 0 || amount1OutSym > 0);

    uint productBeforeSwap = uint(initialReserve0Sym) * uint(initialReserve1Sym);
    uint productAfterSwap = uint(finalReserve0) * uint(finalReserve1);
    assert(productAfterSwap >= productBeforeSwap);
}}