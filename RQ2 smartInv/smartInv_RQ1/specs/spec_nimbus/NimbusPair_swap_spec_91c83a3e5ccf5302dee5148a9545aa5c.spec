...
rule EnsureValidSwap() {
    uint256 amount0Out;
    uint256 amount1Out;
    address to;

    // Initializing symbolic variables to represent token reserves.
    uint112 reserve0;
    uint112 reserve1;

    // Preconditions checks for swap operation.
    require(amount0Out > 0 || amount1Out > 0, "Swap amounts cannot be both zero.");
    require(amount0Out < reserve0 && amount1Out < reserve1, "Swap amounts exceed reserves.");

    address token0;
    address token1;

    // Preventing swap to token addresses.
    require(to != token0 && to != token1, "Invalid swap recipient.");

    uint balance0;
    uint balance1;
    // Mocking balance adjustment pre-swap.
    balance0 = amount0Out > 0 ? balance0 - amount0Out : balance0;
    balance1 = amount1Out > 0 ? balance1 - amount1Out : balance1;

    // Calculating input amounts based on final balance and reserves.
    ...
    // Ensuring there's at least one input amount.
    require(amount0In > 0 || amount1In > 0, "Insufficient input amount.");

    // Simulating referral fee deduction.
    uint refFee0 = amount0In * 3 / 1994;
    uint refFee1 = amount1In * 3 / 1994;
    balance0 = amount0In > 0 ? balance0 - refFee0 : balance0;
    balance1 = amount1In > 0 ? balance1 - refFee1 : balance1;

    // Adjusting balances for fee and validating liquidity.
    uint balance0Adjusted = balance0 * 10000 - amount0In * 15;
    uint balance1Adjusted = balance1 * 10000 - amount1In * 15;

    // Liquidity invariant check.
    assert(balance0Adjusted * balance1Adjusted >= uint(reserve0) * reserve1 * 10000**2);
}}