...
rule TransferFromMaintainsAllowanceInvariant() {
    address $sender;
    address $recipient;
    uint256 $amount;
    address $delegate;

    uint256 allowanceBefore = _allowances[$sender][$delegate];
    transferFrom($sender, $recipient, $amount);

    assert(_allowances[$sender][$delegate] == allowanceBefore - $amount);
}}