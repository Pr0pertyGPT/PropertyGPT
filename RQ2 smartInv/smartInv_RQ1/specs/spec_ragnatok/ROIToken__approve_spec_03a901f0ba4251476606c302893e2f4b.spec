...
rule ApprovalMaintainsAllowanceEquivalence() {
    address $owner;
    address $spender;
    uint256 $amount;

    uint256 allowanceBefore = _allowances[$owner][$spender];
    _approve($owner, $spender, $amount);
    uint256 allowanceAfter = _allowances[$owner][$spender];
    assert((allowanceBefore == 0 && allowanceAfter == $amount) || (allowanceBefore != 0 && allowanceAfter == $amount));
}}