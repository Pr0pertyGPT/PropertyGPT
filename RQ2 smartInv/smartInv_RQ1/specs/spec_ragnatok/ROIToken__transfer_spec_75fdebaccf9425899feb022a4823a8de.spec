...
postcondition{
    to == uniswapV2Pair ? _startTimeForSwap == __old__(_startTimeForSwap) : true;
    (_isExcludedFromFee[from] || _isExcludedFromFee[to]) ? true : (_taxFee == __old__(_taxFee) && _liquidityFee == __old__(_liquidityFee)) ? true : false;
}
}